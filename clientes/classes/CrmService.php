...
class CrmService
{
    ...
    public function criarNegocio($dados, $companyId, $userId)
    {
        ...
        // Validar Consistência Funil vs Etapa
        if ($funilId && $etapaId) {
            $stmtCheck = $this->conn->prepare("SELECT id FROM crm_etapas WHERE id = ? AND funil_id = ? AND company_id = ?");
            $stmtCheck->bind_param("iii", $etapaId, $funilId, $companyId);
            $stmtCheck->execute();
            if ($stmtCheck->get_result()->num_rows === 0) {
                // Fallback: Se etapa não pertence ao funil, pega a primeira etapa do funil correto
                $etapas = $this->listarEtapas($funilId, $companyId);
                if (!empty($etapas)) {
                    $etapaId = array_key_first($etapas); // Usa a primeira etapa válida
                } else {
                    throw new Exception("Funil informado não possui etapas válidas.");
                }
            }
        } elseif ($funilId && !$etapaId) {
            // Se informou funil mas não etapa, pega a primeira
            $etapas = $this->listarEtapas($funilId, $companyId);
            if (!empty($etapas)) {
                $etapaId = array_key_first($etapas);
            } else {
                throw new Exception("Funil informado não possui etapas válidas.");
            }
        } elseif (!$funilId && $etapaId) {
            // Se informou etapa mas não funil, descobre o funil
            $stmtCheck = $this->conn->prepare("SELECT funil_id FROM crm_etapas WHERE id = ? AND company_id = ?");
            $stmtCheck->bind_param("ii", $etapaId, $companyId);
            $stmtCheck->execute();
            $resCheck = $stmtCheck->get_result();
            if ($rowCheck = $resCheck->fetch_assoc()) {
                $funilId = $rowCheck['funil_id'];
            } else {
                throw new Exception("Etapa informada não encontrada.");
            }
        }
    ...
    public function excluirFunil($id, $companyId)
    {
        ...
        $count = $this->conn->query("SELECT COUNT(*) as c FROM crm_negocios WHERE funil_id = $id AND company_id = $companyId")->fetch_assoc()['c'];
        if ($count > 0) {
            throw new Exception("Não é possível excluir funil com negócios ativos.");
        }

        // Excluir etapas primeiro
        $this->conn->query("DELETE FROM crm_etapas WHERE funil_id = $id AND company_id = $companyId");
    ...
    {
        $count = $this->conn->query("SELECT COUNT(*) as c FROM crm_negocios WHERE etapa_id = $id AND company_id = $companyId")->fetch_assoc()['c'];
        if ($count > 0) {
    ...
    public function reordenarEtapas($etapas, $companyId)
    {
        foreach ($etapas as $etapa) {
            ...
            $id = (int) $etapa['id'];
            $this->conn->query("UPDATE crm_etapas SET ordem = $ordem WHERE id = $id AND company_id = $companyId");
        }
        return true;
    }
    ...
    public function vincularEmpresa($companyId, $negocioId, $empresaId, $usuarioId)
    {
        ...
        $res = $this->conn->query("SELECT nome FROM empresas WHERE id = $empresaId");
        $empNome = $res->fetch_assoc()['nome'] ?? '';
    ...
    {
        $this->conn->query("DELETE FROM crm_atividades WHERE negocio_id = $id AND company_id = $companyId");
