...
<script>
        ...
        if (inputBusca) {
            inputBusca.addEventListener('input', function (e) {
                ...
                timeoutBusca = setTimeout(() => {
                    fetch(BASE_URL + 'crm/acoes.php?acao=buscar_contatos&tipo=empresa&q=' + encodeURIComponent(termo))
                        .then(res => res.json())
                        .then(response => {
    ...
    function vincularEmpresa(empresaId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(BASE_URL + 'crm/acoes.php?acao=vincular_empresa', {
                    method: 'POST',
    ...
    function salvarAtividadeModal() {
        ...
        fetch(BASE_URL + 'crm/acoes.php?acao=salvar_anotacao', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                negocio_id: dealId,
    ...
    function carregarNegocio(id) {
        ...
        Promise.all([
            fetch(BASE_URL + 'crm/acoes.php?acao=obter_negocio&id=' + id + '&_t=' + new Date().getTime()).then(r => r.json()),
            fetch(BASE_URL + 'crm/acoes.php?acao=buscar_chat_negocio&negocio_id=' + id).then(r => r.json())
        ])
    ...
    function salvarEdicaoDetalhes() {
        ...
        fetch(BASE_URL + 'crm/acoes.php?acao=editar_negocio', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
<script>
    function excluirNegocio(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(BASE_URL + 'crm/acoes.php?acao=excluir_negocio', {
                    method: 'POST',
                    headers: {
